...
class StlObject:
    ...
    def split( self, nMinNbrTriangleToSplit = 20 ):
        """
        ...
        - nMinNbrTriangleToSplit: give a nimimum of triangle to split to an independant object
        """
        timeBegin = time.time()
        ...
        # if some object left with left than nMinNbrTriangleToSplit, let's concatenate with the bigger object
        prind( "DBG: Stl.split: part3..." )
        numObject1 = 0
        while numObject1 < len(splittedObjectsList):
            #if numObject1 > 3 and (numObject1%4)==0: prind( "DBG: split: %d/%d" % (numObject1,len(splittedObjectsList)) ) 
            
            if len(splittedObjectsList[numObject1].aTriangles) < nMinNbrTriangleToSplit:
                numObject2 = 0
                nIdxMax = 0
                while numObject2 < len(splittedObjectsList):
                    if numObject1 != numObject2 and len(splittedObjectsList[numObject2].aTriangles) >= nMinNbrTriangleToSplit:
                        if len(splittedObjectsList[numObject2].aTriangles) > len(splittedObjectsList[nIdxMax].aTriangles):
                            nIdxMax = numObject2
                    numObject2 += 1
                if numObject1 != nIdxMax:
                    prind( "DBG: elem %d too small and so melted with %d" % (numObject1,nIdxMax) )
                    if numObject1 < nIdxMax:
                        splittedObjectsList[numObject1].aTriangles.extend(splittedObjectsList[nIdxMax].aTriangles)
                        del splittedObjectsList[nIdxMax]
                    else:
                        splittedObjectsList[nIdxMax].aTriangles.extend(splittedObjectsList[numObject1].aTriangles)
                        del splittedObjectsList[numObject1]
                        numObject1 -= 1 # to prevent missing the new elem at idx numObject1
            numObject1 += 1
            
        prind("DBG: duration: %5.2fs (total: %5.2fs)" % (time.time() - timeBegin2,time.time() - timeBegin) ) 
        # sur PepperBackSmall_bottom: 
        # - 133s sur la tablet, 
        # - 129s sur ma machine windows
        # - 85s sur ma machine linux (python2 depuis putty)
